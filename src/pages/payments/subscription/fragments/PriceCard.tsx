      ...
      ${isPopular
            ...
            <div className="mb-8">
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-md ${isPopular ? 'bg-blue-500/20 text-blue-400' : 'bg-slate-100 text-slate-600'}`}>
                        NEW
                    </span>
                </div>
            ...
            <div className="flex items-baseline gap-1 mb-8">
                <span className={`text-2xl font-black ${isPopular ? 'text-white' : 'text-slate-900'}`}>
                    {/* tzs price format */}
                    {Intl.NumberFormat("sw-TZ", { style: "currency", currency: "TZS", maximumFractionDigits: 0 }).format(Number(price))}
                </span>
            ...
            <div className="space-y-4 mb-10">
                {features.map((feature, index) => (
                    <div key={index} className="flex items-start gap-3 group/item">
                        <div className={`mt-0.5 rounded-full p-0.5 transition-colors ${isPopular ? 'bg-blue-500/20 text-blue-400' : 'bg-emerald-100 text-emerald-600'}`}>
                            <IconCheck size={14} stroke={3} />
                        </div>
                        <span className={`text-sm font-medium transition-colors ${isPopular ? 'text-slate-300 group-hover/item:text-white' : 'text-slate-600 group-hover/item:text-slate-900'}`}>
                            {feature}
                        </span>
                    </div>
                ))}
        ...
        ${isPopular
            ...
            <div className="w-full flex items-center justify-center">
                <Button
                    type="button"
                    label={t("subscription-card-button-label", { name })}
            ...
            {/* modals */}
            <SubscriptionPaymentModal open={showModal.open === "subscribe"} onClose={handleModalClose} />
        </div>
    );
};
